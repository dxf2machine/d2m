...
public class myPolyline extends myEntity {
	...
	public myPolyline(String name, int flag, int c, myLayer l,
			Vector<myVertex> v, int visibility, myLineType lineType,
			double thickness) {
	...
	public myPolyline(myPolyline orig) {
		...
		for (int i = 0; i < orig._myVertex.size(); i++) {
			_myVertex.add(new myVertex(orig._myVertex.elementAt(i), true));
		}
		_flag = orig._flag;
	}
	...
	public static myPolyline read(myBufferedReader br, myUnivers univers)
			throws IOException {
		...
		return new myPolyline(name, flag, color, l, lv, visibility, lineType,
